...
export default {
	...
	computed: {
		...
		propsChange() {
			// 引用这几个变量，是为了监听其变化
			let { mode, defaultTime, startYear, endYear, defaultRegion, areaCode, defaultSelector, maxToday } = this;
			return JSON.stringify({ mode, defaultTime, startYear, endYear, defaultRegion, areaCode, defaultSelector, maxToday });
		},
		regionChange() {
			// 引用这几个变量，是为了监听其变化
	...
	methods: {
		...
		setMonths() {
			...
			// 如果启用了maxToday限制，需要检查当前选择的年份是否等于今年
			if (this.maxToday) {
				let today = new Date();
				let currentYear = today.getFullYear();
				let currentMonth = today.getMonth() + 1;
				
				// 如果选择的年份等于当前年份，则限制月份不能超过当前月份
				if (this.year == currentYear) {
					maxMonth = currentMonth;
				}
				// 如果选择的年份大于当前年份，则不显示任何月份选项
				else if (this.year > currentYear) {
					maxMonth = 0;
				}
			}
			
			this.months = this.generateArray(1, maxMonth);
		...
		setDays() {
			...
			// 如果启用了maxToday限制，需要检查当前选择的年月是否超过今天
			if (this.maxToday) {
				let today = new Date();
				let currentYear = today.getFullYear();
				let currentMonth = today.getMonth() + 1;
				...
				// 如果选择的年月等于当前年月，则限制日期不能超过今天
				if (this.year == currentYear && this.month == currentMonth) {
					totalDays = Math.min(totalDays, currentDay);
				}
				// 如果选择的年月大于当前年月，则不显示任何日期选项
				else if (this.year > currentYear || (this.year == currentYear && this.month > currentMonth)) {
					totalDays = 0;
				}
			}
